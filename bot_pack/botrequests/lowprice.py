...
class Lowprice:
    """
    Класс Lowprice: Для отправки на поиск города по запросу юзера по возрастанию.
            Методы:
                find_hotels:
        ...
        try:
            ...
            if in_data[0] < now[0]:
                ...
                if self.id_city is not None:
                    ...
                    self.bot.send_message(self.msg.chat.id, 'Не смог найти такой город, нажмите /reset')
        except Exception:
    ...
    def pag_hotels_low(self, pages):
        try:
            self.hotels.pag_hotels(int(pages))
        except Exception:
            logger.exception('Ошибка при отправки данных из pag_hotels_low в hotels.pag_hotels')
            self.bot.send_message(self.msg.chat.id, 'Програмная ошибка, нажмите /reset')
